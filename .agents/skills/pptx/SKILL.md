...
When creating a deck, also read [Brand theme](#brand-theme-logo-driven) (default palette + logo placement) and [Generated imagery with codex](#generated-imagery-with-codex) (on-brand illustrations, infographics, hero art).
...
## Brand theme (logo-driven)

The brand logo lives at `assets/page-logos.jpg` (relative to this skill's directory) — an orange cat mascot in flat sticker-cartoon style on a red background. Unless the user asks for a different theme, build every deck on this brand:

| Role | Hex | Source |
|---|---|---|
| Primary (dominant, 60-70%) | `D74248` (brand red) | logo background |
| Secondary | `FA8832` (mascot orange) | the cat |
| Accent | `4DBCD8` (sky blue) | laptop screen in logo |
| Light surface / text on dark | `F7F8FA` (off-white) | |
| Shade / depth | `A93836` (deep red) | |

- **Place the logo on the title slide** (a comfortable size, e.g. ~1.2-1.5" square, not a postage stamp) and optionally as a small corner mark (~0.5") on content slides. Insert it with `addImage({ path: ".../assets/page-logos.jpg" })` — the file is square (1024×1024), so keep `w === h` to avoid squashing it.
- The logo's own background is solid red `D74248` — it disappears cleanly into a `D74248` slide background, which is the best-looking placement. On white slides it reads as a red rounded tile; give it whitespace rather than trying to mask the red.
- Use the flat, bold-outline sticker aesthetic as the deck's visual language: flat fills, no gradients, rounded corners, generous outlines. Generated imagery (next section) should match this style — always pass the logo as the style reference.

## Generated imagery with codex

For infographics, diagrams, illustrations, hero images, and any visual PowerPoint can't chart natively, generate images with the `codex` CLI (image generation is enabled on this machine). Do **not** use it for data charts — those stay native via `addChart()`.

Invocation pattern (the flag order matters — `-i` is greedy and will swallow a trailing prompt as a filename, so pipe the prompt through stdin and end with `-`):

```bash
cd <dir-where-image-should-land>
echo "Generate one illustration in the same flat sticker-cartoon art style as the attached logo (bold outlines, flat colors, red/orange/blue palette): <WHAT YOU WANT>. Save it as <name>.png in the current directory. 16:9 landscape." \
  | codex exec --skip-git-repo-check -s workspace-write \
      -i <skill-dir>/assets/page-logos.jpg -
```

- **Always attach the logo with `-i`** and ask for "the same flat sticker-cartoon art style as the attached logo" — this is what keeps generated art on-brand and consistent across slides.
- State the aspect ratio in the prompt (`16:9 landscape` for half/full-bleed art, `square` for icons and spot illustrations) and the exact output filename, then verify the file exists afterward — codex is an agent, not a function call, and can occasionally save under a different name (`ls` the directory if the expected file is missing).
- **Specify the background color in the prompt.** With the logo as style reference, codex defaults to the logo's red backdrop. That's ideal on a `D74248` slide; for any other slide say so explicitly (e.g. "on a plain white background") or the image lands as a mismatched red rectangle.
- A generation takes on the order of a minute. When a deck needs several images, launch the codex calls **in parallel as background jobs** and build the rest of the deck while they run.
- For a diagram with text labels (architecture boxes, flows), prefer building it from pptxgenjs shapes — generated images misspell labels. Use codex for label-free or decorative diagram art, and overlay real text with pptxgenjs on top if labels are needed.
- Insert results with `addImage({ path: "..." })`, sized to fill their layout slot (see gap-avoidance below).
...
- Breathing room means *even* padding around content — not dead zones. A slide whose content huddles in the top half with an empty bottom third looks unfinished. Plan each slide's layout to distribute content across the full canvas: if a slide has little content, scale it up (bigger type, bigger visual, more whitespace *around* everything equally) or merge it with a neighbor — don't leave one large empty region.
- After laying out a slide, sanity-check the bounding box of everything on it: content should span most of the usable area (roughly y = 0.5" to slide height − 0.5"). If the last element ends 2"+ above the bottom margin, either the visual element should grow to fill the remaining band or the vertical spacing should be redistributed.
- Half-bleed images and background shapes are the easiest gap-fillers: extend them to the actual slide edge (0 or full width/height), not to an arbitrary stopping point that strands a gutter of background.
...
- **Dead zones** — any empty region taller/wider than ~2" with nothing in it (commonly the bottom third, or the right half of a slide whose text wrapped narrower than planned). Fix by enlarging the slide's visual, redistributing vertical spacing, or extending a background shape to the slide edge
